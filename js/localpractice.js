    ...
    allNavItems.forEach(item => {
        if (!item.classList.contains('nav-parent')) {
            item.addEventListener('click', function() {
                const parentTab = item.getAttribute('data-parent');
    
                // Close all other submenus except the one this belongs to
                document.querySelectorAll('.nav-sub-menu').forEach(menu => {
                    const siblingParent = menu.previousElementSibling?.getAttribute('data-tab');
                    if (siblingParent !== parentTab) {
                        menu.classList.remove('show');
                    }
                });
    
                // Optionally: highlight the active sub-item
                document.querySelectorAll('.nav-item.sub').forEach(sub => sub.classList.remove('active'));
